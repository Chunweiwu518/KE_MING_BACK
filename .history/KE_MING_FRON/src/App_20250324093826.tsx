...
function App() {
        ...
        try {
          ...
          for (const line of lines) {
            ...
            if (data.startsWith('[SOURCES]') && data.endsWith('[/SOURCES]')) {
              try {
                // 解析來源數據前先進行一次解碼
                const sourcesData = decodeURIComponent(
                  data.replace('[SOURCES]', '').replace('[/SOURCES]', '')
                )
  ...
  // 添加一個來源資料的解析輔助函數
  const parseSources = (sourcesData: string): Source[] => {
    try {
      const decodedData = decodeURIComponent(sourcesData)
      const sources = JSON.parse(decodedData)
      return Array.isArray(sources) ? sources : []
    } catch (e) {
      console.error('解析來源資料失敗:', e)
      return []
    }
  }
                        ...
                        <div key={sourceIndex} className="p-4 hover:bg-gray-50 transition-colors">
                          ...
                          <div className="text-sm text-gray-700 bg-white p-3 rounded-lg border border-gray-100">
                            {source.content ? (
                              ...
                              <p className="text-gray-500 italic">無法顯示內容</p>
                            )}
                          </div>
                        </div>
                      ))}
