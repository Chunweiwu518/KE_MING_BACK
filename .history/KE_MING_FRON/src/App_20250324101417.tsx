...
function App() {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/chat/stream`, {
        ...
        body: JSON.stringify({
          ...
          history: updatedMessages.slice(0, -1).map(msg => ({  // 不包含最後一條空的助手消息
            role: msg.role,
            content: msg.content
          }))
        })
  ...
  const deleteAllHistory = async () => {
    try {
      ...
      console.error('刪除所有對話歷史失敗:', error)
      setError('刪除所有對話歷史失敗')
    }
  }
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* 側邊欄 */}
      <div className={`${sidebarOpen ? 'w-72' : 'w-0'} bg-white shadow-lg transition-all duration-300 overflow-hidden flex flex-col`}>
        {/* 頂部標題區域 */}
        ...
        <div className="p-4 border-b bg-gray-50">
          <button
            ...
            className="w-full bg-purple-600 text-white rounded-lg px-4 py-2 hover:bg-purple-700 transition-colors flex items-center justify-center"
          >
        ...
        <div className="p-4 border-b bg-gray-50 flex space-x-2">
          <button
            ...
            className="flex-1 bg-green-600 text-white rounded-lg px-3 py-2 hover:bg-green-700 transition-colors flex items-center justify-center"
          >
          ...
          <button
            onClick={() => {
              if (confirm('確定要刪除所有對話歷史嗎？此操作無法恢復。')) {
                deleteAllHistory()
              }
            }}
            className="flex-1 bg-red-600 text-white rounded-lg px-3 py-2 hover:bg-red-700 transition-colors flex items-center justify-center"
          >
            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {chatHistories.map((chat) => (
            <div
              ...
              className={`group relative hover:bg-gray-50 transition-colors ${
                currentChatId === chat.id ? 'bg-purple-50 border-l-4 border-purple-600' : 'border-l-4 border-transparent'
              }`}
            >
              <div 
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-2 text-red-600 hover:text-red-800"
              >
